...
public class LinkedControllerServerHandler {
    ...
    public static WorldAttached<Map<UUID, Collection<ManualFrequencyEntry>>> receivedInputs = new WorldAttached<>(
            $ -> new HashMap<>());
    ...
    static class ManualFrequencyEntry extends IntAttached<Couple<Frequency>> implements IRedstoneLinkable {
        ...
        @Override
        public Pair<String, String> getNetworkKey() {
            return Pair.of(getSecond().getFirst().itemName, getSecond().getSecond().itemName);
        }
